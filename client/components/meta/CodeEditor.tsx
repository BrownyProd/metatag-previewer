...
function highlightHtml(code: string) {
  const esc = code
    ...
    .replace(/>/g, "&gt;");
  return esc
    .replace(/(&lt;!--[\s\S]*?--&gt;)/g, '<span class="text-emerald-400/80">$1</span>')
    .replace(/(&lt;\/?)([a-zA-Z0-9:-]+)([^&]*?)(\/?&gt;)/g, (_m, p1, p2, p3, p4) => {
      const attrs = p3.replace(/([a-zA-Z-:]+)(=)("[^"]*"|'[^']*')/g, '<span class="text-cyan-300">$1</span>$2<span class="text-amber-300">$3</span>');
...
export function CodeEditor({ value, onChange, className, placeholder, onFileLoad }: CodeEditorProps) {
  ...
  return (
    <div className={cn("group relative w-full rounded-xl border border-white/10 bg-white/5 dark:bg-black/20 backdrop-blur-xl shadow-[inset_0_1px_0_0_rgba(255,255,255,0.06)]", className)}>
      <div className="flex items-center justify-between px-3 py-2 text-xs text-foreground/70">
        ...
        <div className="flex items-center gap-2">
          <button onClick={openFile} className="rounded-md bg-white/10 px-2 py-1 hover:bg-white/15 transition">Upload</button>
          <input ref={fileRef} type="file" accept=".html,.htm,.txt" className="hidden" onChange={onFile} />
        </div>
      </div>
